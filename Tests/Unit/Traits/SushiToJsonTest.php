...
class SushiToJsonTest extends TestCase
{
    ...
    public function it_loads_valid_json_data_correctly(): void
    {
        ...
        $directory = dirname($this->testJsonPath);
        File::makeDirectory($directory, 0755, true, true);
        File::put($this->testJsonPath, \Safe\json_encode($testData, JSON_PRETTY_PRINT));
    ...
    public function it_normalizes_nested_arrays_in_json_data(): void
    {
        ...
        $metadataDecoded = \Safe\json_decode($rows['1']['metadata'], true);
        Assert::isArray($metadataDecoded);
        $tagsDecoded = \Safe\json_decode($rows['1']['tags'], true);
        Assert::isArray($tagsDecoded);
        expect($metadataDecoded)->toBe(['nested' => ['deep' => 'value']]);
        expect($tagsDecoded)->toBe(['tag1', 'tag2']);
    }
    ...
    public function it_saves_data_to_json_file_successfully(): void
    {
        ...
        $savedData = \Safe\json_decode($savedContent, true);
        Assert::isArray($savedData);
    ...
    public function it_handles_updating_event_correctly(): void
    {
        ...
        // Mock dell'utente autenticato
        $user = Mockery::mock('stdClass');
        $user->shouldReceive('offsetGet')->with('id')->andReturn(789);
        $user->id = 789;
    ...
    public function it_works_with_sushi_package_integration(): void
    {
        ...
        $directory = dirname($this->testJsonPath);
        File::makeDirectory($directory, 0755, true, true);
        $jsonContent = \Safe\json_encode($testData, JSON_PRETTY_PRINT);
        File::put($this->testJsonPath, $jsonContent);
