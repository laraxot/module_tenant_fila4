...
## Path dei file tenant (source of truth)

Il tenant viene risolto come **path** (non come dominio testuale) usando reverse-domain.

Esempio:

- Dominio: `laravelpizza.local`
- Tenant name: `local/laravelpizza`
- Directory config: `config/local/laravelpizza/`

Questa gerarchia (`config/{environment}/{tenant}`) permette di avere lo stesso tenant su più environment (local/staging/production) senza duplicare naming o accoppiare environment nel nome.
